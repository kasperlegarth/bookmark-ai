...
async function addBookmark() {
  if (localStep.value === 1) {
    const isValidUrl = await validateUrl(localUrl.value);
    if (!isValidUrl) {
    ...
    if(localMode.value === 'edit') {
      bookmarkStore.updateBookmarkById(localEditId.value, {
        ...
        tags: localTags.value.split(',').map(tag => tag.trim()), 
      })
    } else {
    ...
    <Dialog v-model:open="localDialogOpen">
      ...
      <DialogContent class="sm:max-w-[425px] grid-rows-[auto_minmax(0,1fr)_auto] px-0 max-h-[90dvh]">
        <DialogHeader class="px-6">
          <DialogTitle><span v-if="localMode == 'add'">Add</span><span v-if="localMode == 'edit'">Update</span> Bookmark</DialogTitle>
        ...
        <form id="addForm" @submit="handleSubmit($event, addBookmark)" class="grid gap-4 py-4 px-6 overflow-y-auto">
          <FormField v-slot="{ componentField }" name="url">
            <FormItem class="relative">
              ...
              <FormControl>
                <Input id="url" type="text" :defaultValue="localUrl" v-model="localUrl" @input="handleUrlInput(localUrl)" placeholder="https://example.com" v-bind="componentField" />
          ...
          <div class="expand-reveal space-y-4" :class="{ expanded: localStep === 2 }">
            <FormField v-slot="{ componentField }" name="siteName">
              <FormItem>
                ...
                <FormControl>
                  <Input id="site" type="text" :defaultValue="localSiteName" v-model="localSiteName" placeholder="Site Name" v-bind="componentField" />
                </FormControl>
                <FormDescription>
            ...
            <FormField v-slot="{ componentField }" name="description">
              <FormItem>
                ...
                <FormControl>
                  <Textarea id="description" type="text" :defaultValue="localDescription" v-model="localDescription" placeholder="Description" v-bind="componentField" />
                </FormControl>
                <FormDescription>
            ...
            <FormField v-slot="{ componentField }" name="tags">
              <FormItem>
                ...
                <FormControl>
                  <Input id="tags" type="text" :defaultValue="localTags" v-model="localTags" placeholder="Tags" v-bind="componentField" />
                </FormControl>
                <FormDescription>
        ...
        <DialogFooter class="px-6">
          ...
          <Button type="submit" form="addForm">
            ...
            <template v-if="localStep === 2"><span v-if="localMode == 'add'">Add</span><span v-if="localMode == 'edit'">Update</span> Bookmark</template>
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
